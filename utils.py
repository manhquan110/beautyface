...
def CropAndResize(indir, output_size, label):
    ...
    for index, row in label.iterrows():
        label['Files'][index] = str(os.path.join(
            outdir, row['Files'].split('/')[-1]))
        t = threading.Thread(
            target=ImgProcess, args=(row, outdir, output_size,))
        t.start()
...
def LoadandPreprocessImg(path):
    ...
    new_img = Image.new('RGB', (oldsize[1], oldsize[1]), (255, 255, 255))
    new_img.paste(img)
    new_img.thumbnail((227, 227), Image.ANTIALIAS)
...
def getRealPath(path):
    return os.path.join(os.getcwd(), 'dataset/'+os.path.normpath(path))
